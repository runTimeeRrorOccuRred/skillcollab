// ignore_for_file: deprecated_member_use, eol_at_end_of_file, non_constant_identifier_names, prefer_const_constructors, require_trailing_commas
...
class UserPostTile extends ConsumerStatefulWidget {
  final PeopleAndGroupPost userPostData;
  final int index;
  final bool? isSearch;
  const UserPostTile({super.key, required this.userPostData, required this.index, this.isSearch = false});

  @override
  ConsumerState<ConsumerStatefulWidget> createState() => _UserPostTileState();
}

class _UserPostTileState extends ConsumerState<UserPostTile> {
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: !(widget.userPostData.userId?.profilePhoto
                              ...
                              : ClipRRect(
                                  borderRadius: BorderRadius.circular(50),
                                  child: Image.network( widget.userPostData.userId?.profilePhoto ?? "",
                                    fit: BoxFit.cover,
                                  ),),
                        ),
                      ),
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            // Text( "posted on ${DateFormat.E().format(widget.userPostData.createdAt ?? DateTime.now())} at ${DateFormat.jm().format(widget.userPostData.createdAt ?? DateTime.now())}",),
                            Row(
                              children: [
                                const Icon(
                          ...
                          children: [
                            GestureDetector(
                                        ...
                                        actions: [
                                          ...
                                          TextButton(
                                            onPressed: () async {
                                              ...
                                              if(widget.isSearch ?? false) {
                                                await ref.read(searchViewModel).unfollowUser(context, widget.userPostData.userId?.id ?? "").then((value) {
                                                  final list = ref.read(searchViewModel).allDataList.map((e) {
                                                    PeopleAndGroupPost updatedPostData = PeopleAndGroupPost();
                                                    if((e.userId?.id ?? "") == (widget.userPostData.userId?.id ?? "")) {
                                                      updatedPostData = e.copyWith(
                              ...
                              child: Container(
                                ...
                                child: Image.asset("assets/icons/follow.png", color: Colors.white,),
                              ),
                            ),
                            gapW10,
                          ],
