...
import 'package:skill_colab/data/remote/groups/getGroup/models/get_group_model.dart' as ggm;
...
import 'package:skill_colab/data/remote/groups/groupAccess/group_access_repo_impl.dart';
...
import 'package:skill_colab/view/screens/groups/premiumGroupProfile/subscribe_payment_webview.dart';
import 'package:video_player/video_player.dart';

final premiumGroupProfileViewModel = ChangeNotifierProvider((ref) => PremiumGroupProfileViewModel(ref: ref));

class PremiumGroupProfileViewModel extends BaseViewModel<BaseScreenView> {
  Ref ref;
  PremiumGroupProfileViewModel({required this.ref}) : super();
  ...
  final GroupAccessRepo _groupAccessRepoImpl = GroupAccessRepoImpl(); 
  ...
  ggm.GetGroupResponseModel _getGroupResponseModel = const ggm.GetGroupResponseModel();
  GroupSubscribeResponseModel _groupSubscribeResponseModel = const GroupSubscribeResponseModel();
  GetCustomerPlanResponseModel _getCustomerPlanResponseModel = const GetCustomerPlanResponseModel();
  GroupUnubscribeResponseModel _groupUnubscribeResponseModel = const GroupUnubscribeResponseModel();
  ...
  GroupAccessResponseModel _groupAccessResponseModel = const GroupAccessResponseModel();
  ...
  GroupAccessResponseModel get groupAccessResponseModel => _groupAccessResponseModel;
  ...
  ggm.GetGroupResponseModel get getGroupResponseModel => _getGroupResponseModel;
  UserFollowStatusResponseModel get userFollowStatusResponseModel => _userFollowStatusResponseModel;
  GroupSubscribeResponseModel get groupSubscribeResponseModel => _groupSubscribeResponseModel;
  GetCustomerPlanResponseModel get getCustomerPlanResponseModel => _getCustomerPlanResponseModel;
  GroupUnubscribeResponseModel get groupUnubscribeResponseModel => _groupUnubscribeResponseModel;
 ...
 Future<void> groupTips(BuildContext context, groupTipsRequestModel, String consumerId, int amount) async {
  ...
  Future<void> getGroupRating(BuildContext context, String groupId, {int rating = 0,}) async {
    clearAllData();
  ...
  Future<void> getGroupDataFromApi(BuildContext context, String groupId, {bool load = true}) async {
    if(load) toggleLoading();
    await _getGroupRepoImpl.getGroup(groupId).then((value) {
      ...
      return value.fold(
        ...
        getGroupPlan(GetCustomerPlanRequestModel(product: r.data?.productId ?? ""));

        notifyListeners();
        
        // final Map<String, dynamic> groupdata = {
        //   ...r.data?.toJson() ?? {},
        //   "userId": r.data?.userId?.toJson() ?? {},
        //   "follower": r.data?.followers ?? 0,
        //   "interests": r.data?.interests?.map((e) => e.toJson()).toList() ?? [],
        //   "subscribers": []
        // };

        // for(int i = 0; i < (r.data?.subscribers?.length ?? 0); i++) {
        //   Map<String, dynamic> sub = {
        //     ...r.data?.subscribers?[i].toJson() ?? {},
        //     "user": r.data?.subscribers?[i].user?.toJson() ?? {}
        //   };
        //   groupdata['subscribers'].add(sub);
        // }      
  ...
  String formatSubscribers(int number) {
    return number == 1 ? 'Subscriber' : 'Subscribers';
  }
  ...
  Future<void> getUserFollowStatus(String groupId, {bool load = true}) async {
    if(load) toggleLoading();
  ...
  Future<void> subscribeGroup(BuildContext context, GroupSubscribeRequestModel groupSubscribeRequestModel, String customerId) async {
    toggleLoading();
    _groupSubscribeRepoImpl.subscribeGroup(groupSubscribeRequestModel, customerId).then((value) {
      toggleLoading();
      return value.fold(
        (l){
          context.pop();
        ...
        (r) {
          _groupSubscribeResponseModel = r;
          getGroupDataFromApi(context, groupSubscribeRequestModel.groupId ?? "");
          context.pop();
          Logger.printSuccess(_groupSubscribeResponseModel.toString());
          notifyListeners();
        }
      );
    });
  ...
  Future<void> unsubscribeGroup(BuildContext context, GroupUnubscribeRequestModel groupUnsubscribeRequestModel, String customerId) async {
    toggleLoading();
    _groupSubscribeRepoImpl.unSubscribeGroup(groupUnsubscribeRequestModel, customerId).then((value) {
      toggleLoading();
      return value.fold(
        (l) => Logger.printError(l.message), 
        (r) {
          _groupUnubscribeResponseModel = r;
          getGroupDataFromApi(context, groupUnsubscribeRequestModel.groupId ?? "");
          Logger.printSuccess(r.toString());
          Navigator.pop(context);
        }
      );
  ...
  Future<void> getGroupPlan(GetCustomerPlanRequestModel getCustomerPlanRequestModel) async {
    _planCreationRepoImpl.getCustomerPlan(getCustomerPlanRequestModel).then((value) {
      return value.fold(
        (l) => Logger.printError(l.message), 
        (r) {
          _getCustomerPlanResponseModel = r;
          Logger.printSuccess(_getCustomerPlanResponseModel.toString());
          notifyListeners();
        }
      );
    });
    ...
    await _customerSubscribeRepoImpl.customerSubscribeStripeLink(customerSubscribeRequestModel, customerId).then((value) {
      ...
      return value.fold(
        ...
        (r) {
          ...
          notifyListeners();
          Logger.printSuccess(_customerSubscribeResponseModel.toString());

          Navigator.push(context, MaterialPageRoute(builder: (context) => SubscribePaymentWebview(paymentUrl: r.data?.sessionUrl ?? "",))).then((value) {
            Navigator.pop(context);
          });
        }
      );
  ...
  Future<void> groupAccess(BuildContext context, GroupAccessRequestModel groupAccessRequestModel) async {
    await _groupAccessRepoImpl.groupAccess(groupAccessRequestModel).then((value) {
      return value.fold(
        (l) => Logger.printError(l.toString()), 
        (r) {
          _groupAccessResponseModel = r;
          Logger.printSuccess("GroupAccessContent $_groupAccessResponseModel");
          notifyListeners();
        }
      );
    });
